...
def _project_header_label(project_code: str | None, project_name: str | None) -> str | None:
    return " ".join(part for part in (project_code, project_name) if part) or None
...
def export_meeting_minutes(payload: MeetingMinutesPayload):
    project_code, project_name = _project_export_details(payload)
    docx_bytes = build_meeting_minutes_docx(payload, _project_header_label(project_code, project_name))
