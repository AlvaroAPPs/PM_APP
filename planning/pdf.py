...
def _page_header(page: list[str], logo: PdfImage | None, project_name: str, project_code: str, generated_at: date) -> None:
...
def build_gantt_pdf(project_name: str, project_code: str, items: list[dict]) -> bytes:
    ...
    if not parsed_items:
        page: list[str] = []
        _page_header(page, logo, project_name, project_code, generated_at)
    ...
    for chunk in chunks:
        ...
        for item, depth in chunk:
            ...
            pdf_text(page, CONTENT_X, y - 11, title[:40], size=7.5, color_rgb=INK, bold=(depth == 0 and item["source"] != "manual"))
            ...
            if item["is_milestone"]:
                ...
                pdf_rect(page, bx, y - ROW_H / 2 - bar_h / 2, max(3.0, bx_end - bx), bar_h,
                          fill_rgb=color, stroke_rgb=None)
            y -= ROW_H
